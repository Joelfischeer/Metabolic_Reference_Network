...
run_network.py
==============
Condition-specific bootstrapped metabolic edge network, using an
Otsuka–Ochiai (cosine) coefficient for edge strength instead of a raw
cross-mention count.

For each organ-organ pair the pipeline:
  1. Queries PubMed:
         (MeSH_A OR aliases_A)
     AND (MeSH_B OR aliases_B)
     AND METABOLIC_FILTER
     AND CONDITION_FILTER
  2. Fetches up to MAX_PAPERS abstracts
  3. Keeps papers where both organs co-occur in the same sentence / compound
...
Run from the Metabolic_Reference_Network/ directory:
    uv run -m Edge_cosine_met_reference_network.run_network --condition healthy
    uv run -m Edge_cosine_met_reference_network.run_network --condition obese
...
HERE = Path(__file__).parent          # Edge_cosine_met_reference_network/
...
def build_query(organ1: str, organ2: str,
                ...
                metabolic_filter: str, condition_filter: str,
                crosstalk_filter: str) -> str:
    c1 = _alias_clause(organ1, organ_aliases, organ_mesh)
    c2 = _alias_clause(organ2, organ_aliases, organ_mesh)
    return f"{c1} AND {c2} AND {metabolic_filter} AND {condition_filter} AND {crosstalk_filter}"
...
def build_per_organ_query(organ: str, organ_aliases: dict, organ_mesh: dict,
    ...
    return f"{clause} AND {metabolic_filter} AND {condition_filter} AND {crosstalk_filter}"
...
def run_search(organs: list[str], output_path: Path,
               organ_aliases: dict, organ_mesh: dict,
               metabolic_filter: str, condition_filter: str,
               crosstalk_filter: str, crosstalk_keywords: list[str], cfg,
               resume: bool = True) -> dict:
    """
    Per-organ PubMed search. Returns {organ: {query, pmids, papers, n_found}}.

    The PubMed query's crosstalk clause only requires a crosstalk keyword to
    ...
    for idx, organ in enumerate(organs):
        if organ in results:
            r = results[organ]
            print(f"  [{idx+1}/{total}] Cached {organ}: {r.get('n_found', 0)} papers")
            continue

        query = build_per_organ_query(organ, organ_aliases, organ_mesh,
        ...
        pmids      = search_pubmed(query, cfg.MAX_PAPERS, cfg.YEARS_BACK)
        n_raw      = len(pmids)
        print(f"    Found {n_raw} PMIDs", end="", flush=True)
        ...
        if n_raw == 0:
            ...
            papers_raw = fetch_abstracts(pmids, delay=cfg.DELAY)
...
def run_bootstrap(search_results: dict, output_path: Path, cfg,
                  ...
                  resume: bool = True) -> dict:
    """
    Per iteration: sample SAMPLE_FRACTION of each organ's paper pool and count
    how often each other organ is mentioned.  The mean cross-mention count is
    then normalized into an Otsuka–Ochiai coefficient — mean / sqrt(n_found_o1
    * n_found_o2) — so an organ with a much larger literature pool doesn't
    dominate every edge just by volume.  mean Otsuka–Ochiai coefficient
    >= MIN_BOOTSTRAP_MEAN → robust edge.  Papers are stored in the boot dict.
    """
    ...
    # Pre-build word-boundary/negation-safe alias patterns for fast text search
    ...
    # Pre-compute key-player terms for all papers across all organ pools
    ...
    # Fast pmid → paper lookup for visualization
    ...
    for it in range(cfg.N_BOOTSTRAP):
        # Sample a fraction of each organ's PMID pool
        ...
        # Count cross-organ mentions per pair
        for (o1, o2), key in zip(pairs, pair_keys):
            ...
            # Key player accumulation for this iteration
    ...
    for (o1, o2), key in zip(pairs, pair_keys):
        ...
        n_h = len(kp.get("hormones", []))
        n_m = len(kp.get("metabolites", []))
        n_p = len(kp.get("proteins", []))
        print(f"  {flag} OO={oo_mean:.5f} ± {oo_std:.5f}  cv={cv:.3f}  "
              f"(raw mean={raw_mean:.1f})  kp: {n_h}H/{n_m}M/{n_p}P  ({o1} <-> {o2})")
...
def _build_overview_html_string(boot: dict, cfg) -> str:
    ...
    elbow plot) as a string. Used both to write the standalone file and to
    embed the same content as a tab in the robust-network dashboard."""
...
<div class="card">
  ...
  <div class="card-hint">Sorted by mean Otsuka–Ochiai coefficient. Error bars = ± 1 SD.</div>
...
if(BP.length===0){{
  ...
  BP.forEach((p,i)=>{{
    ...
    bx.fillStyle=robust?'rgba(14,165,233,0.85)':'rgba(14,165,233,0.38)';
    bx.fillRect(LFT,y,sc(p.mean),BH);
    if(p.std>0){{
...
def build_robust_network(boot: dict, search_results: dict, output_path: Path, cfg,
    ...
    try:
        ...
        for edge_key, b in boot.items():
            ...
            # Build per-category term lists and integer-percentage count dicts
            # for the viz (counts dict drives the badge: N% in bootstrap mode)
            ...
            q1 = search_results.get(o1, {}).get("query", "")
            q2 = search_results.get(o2, {}).get("query", "")
            description = (
                f"{cfg.VIZ_LABEL} metabolic connection — {o1} ↔ {o2}\n"
...
def _robust_info_tabs(cfg, organs: list[str], search_results: dict, boot: dict) -> list[dict]:
    ...
    <p class="info-p">
      ...
      in the <strong>{cfg.VIZ_LABEL.lower()}</strong> literature context.
    </p>
    <div class="info-h2">Per-Organ Query</div>
...
AND CONDITION_FILTER ({cfg.VIZ_LABEL})</div>
    ...
    <p class="info-p">
      An organ with a much larger literature pool doesn't dominate edges just
      by volume. Edges reflect how specifically two organs are tied together in
      the literature.
    </p>
    ...
    <div class="info-stat-grid">
      ...
      <div class="info-stat"><div class="info-stat-val">{cfg.MAX_PAPERS:,}</div><div class="info-stat-lbl">Max papers/organ</div></div>
    ...
    <p class="info-p">
      ...
      <strong>proteins</strong> with their bootstrap frequency which is the fraction of
...
""",
    ...
    <p class="info-p">
      One PubMed query is run per organ, ANDing the organ terms with the
      metabolic filter, the condition filter, and the crosstalk filter below.
      Up to {cfg.MAX_PAPERS:,} papers are fetched per organ. During bootstrapping,
      papers from one organ's pool that mention another organ in their
      title/abstract are counted as cross-organ evidence. Both organs have
      to appear in the same sentence. As normalization the Otsuka–Ochiai
      coefficient is used so a heavily studied organ's total literature pool
      size (n_found) doesn't dominate purely by volume.
    </p>
    <p class="info-p">
    ...
    <p class="info-p">
      ...
      Ensures every retrieved paper is about metabolism, not just anatomy.
    </p>
    <div class="info-code">{metabolic_kws}</div>
    <div class="info-h2">CONDITION_FILTER — {cfg.VIZ_LABEL}</div>
    <p class="info-p">
      At least one of these keywords must also appear, restricting results to the
    ...
    <p class="info-p">
      At least one of these keywords must also appear somewhere in the
      document (the PubMed query's coarse pre-filter) — restricting results
      to papers actually discussing inter-organ relationships, not just
      papers that happen to mention an organ and a metabolic term without
      relating them to anything else. After fetching, papers are filtered
      further: kept only if the organ's own name/alias and one of these
      crosstalk terms appear together in the <strong>same sentence</strong>
...
def main():
    ...
    print(f"[i] Max papers: {cfg.MAX_PAPERS:,}/organ | Years back: {cfg.YEARS_BACK}")
    ...
    if not args.viz_only:
        search_results = run_search(
            ...
            metabolic_filter, condition_filter, crosstalk_filter,
            cfg.CROSSTALK_KEYWORDS, cfg,
