# =============================================================================
#  EDGE COSINE METABOLISM REFERENCE NETWORK  (HEALTHY CONDITION)
#  Configuration file
#
...
#      uv run -m Edge_cosine_met_reference_network.run_network --condition healthy --viz-only
#
#  To wipe cache and restart from scratch:
#      uv run -m Edge_cosine_met_reference_network.run_network --condition healthy --reset
# =============================================================================
...
YEARS_BACK  = 30     # How many years back to search in PubMed.
...
MIN_COOCCUR = 1      # Minimum co-occurring papers to include a pair in the
...
# -----------------------------------------------------------------------------
#  Condition-specific keyword filter  (HEALTHY)
# -----------------------------------------------------------------------------
#  At least ONE of these must appear in title/abstract in addition to the
#  metabolic filter above.
#
#  Goal: restrict papers to those studying healthy / normal physiology —
#  no disease state, lean subjects, normal glucose regulation.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  At least ONE of these must appear in title/abstract in addition to the
#  metabolic and condition filters above.
#
...
#  organ name and a metabolic term without relating them to each other.
# -----------------------------------------------------------------------------
...
MIN_BOOTSTRAP_MEAN = Elbow   # mean Otsuka–Ochiai coefficient threshold.
                              ...
                              # run (see bootstrap_overview_healthy.html).
...
VIZ_TITLE_OVERVIEW = "Healthy Cosine Metabolic Network — Bootstrap Strength Overview"
VIZ_TITLE_ROBUST   = "Healthy Cosine Metabolic Network — Robust Connections"
...
#  Edge_cosine_met_reference_network/run_llm_descriptions.py.
#
