...
Run from the Metabolic_Reference_Network/ directory:
    uv run -m Edge_cosine_general_reference_network.run_comparison --condition healthy
    uv run -m Edge_cosine_general_reference_network.run_comparison --condition obese
...
def load_network_edges(condition: str, cfg, allowed_organs: set[str]) -> dict:
    ...
    if not boot_json.exists():
        ...
        print(f"    Run:  uv run -m Edge_cosine_general_reference_network.run_network "
...
def _organ_breakdown_tab(categories: dict, cfg) -> dict:
    ...
    <p class="info-p">
      ...
      reference network</strong>. Bar length scales with each organ's total
      connection count; the trailing numbers are Shared / Only-Literature / Only-Reference.
    </p>
